...
1. Attempt a real Ruflo research workflow: discover the registered Ruflo MCP
   tools and their current schemas, invoke `workflow_run` with a research
   template (or the closest verified one), record the workflow ID, and
   perform at least one bounded `workflow_status` check.
2. Known limitation: Ruflo has consistently stalled at 0% with a pending
   ...
   ID and last-known status, then fall back to native research for the same
   question. Never represent native findings as Ruflo output.
