...
const sections: UniversalSections = {
  ...
  mainContent: [
    "**Written authorization comes first, even in a lab you own.** The rules-of-engagement document doesn't exist to satisfy a client — it exists so that what you did, when, and to what, is on the record rather than reconstructed from memory afterward. At minimum it names: who authorized the exercise (for a personal lab, this can be a one-paragraph self-authorization dated and kept alongside the lab's own documentation; for a team or organizational lab, it needs a named approver with actual authority over the environment); the exact systems in scope, by name or address range; an exact start and end date/time for the exercise; and the specific class of activity permitted (for example, reconnaissance and non-destructive exploitation of intentionally vulnerable lab targets) versus anything requiring separate authorization. NIST SP 800-115 treats this planning step — fixing scope, objectives, and rules of engagement before any technical activity begins — as the foundation the rest of a security test depends on, and that holds whether the target is a client's production estate or your own lab.",
    "**The out-of-scope list matters as much in a lab as anywhere else.** Write down, explicitly, what the exercise must not touch: your own workstation if it isn't part of the intentionally isolated environment, any account or service that also has a real-world purpose, any shared network segment the lab happens to sit near, and any system you don't have sole administrative control over. A lab environment that was assembled quickly often has more of these adjacent, unintentionally-reachable systems than its operator expects — which is exactly why the next section treats isolation as something to verify, not assume.",
    "**The rules of engagement need a time window and an emergency-stop procedure, not just a target list.** Set an explicit start and end time for the exercise rather than leaving it open-ended — an exercise with no defined end tends to quietly expand in scope as 'just one more thing.' Separately, write down what happens if something goes wrong: the lab's hypervisor host becomes unresponsive, a virtual machine's network configuration turns out to bridge somewhere unexpected, or a script behaves in a way that wasn't planned. The procedure should be as simple as 'stop the exercise, power down or disconnect the affected lab segment, and do not resume until the cause is understood' — the point isn't sophistication, it's having decided this in advance rather than improvising under pressure, following the same emergency-contact-and-stop discipline PTES's pre-engagement framework describes for client engagements.",
    ...
    "**Explicit written sign-off closes the authorization loop.** For a solo lab, this can be as simple as a dated, versioned document alongside the lab's own configuration, stating what you authorized yourself to do and when — trivial to write, and the single artifact that turns 'I was just testing my own stuff' from an unverifiable claim into a documented fact. For a team or organizational lab, sign-off needs to come from whoever actually has the authority to authorize testing of every system the lab touches, the same requirement client-engagement scoping depends on. Either way, testing does not start until this document exists and is dated before the exercise, not written up afterward to match what happened.",
