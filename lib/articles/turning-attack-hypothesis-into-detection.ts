...
const sections: UniversalSections = {
  ...
  mainContent: [
    ...
    "SIMULATED true positive — host ALD-WKS-2214, user m.romero: a spawned `cmd.exe` process (parent: a document-viewer process, not an approved deployment tool) runs `schtasks.exe` to create task \"WinUpdaterHelper\", targeting an executable named `svcupd.exe` that was written moments earlier into the user's per-account temporary-files directory (i.e. the kind of location every desktop OS reserves per-user for transient app data — never an allow-listed installation directory) and launches at logon under m.romero's own account — consistent with the threat model's user-level attacker: creating a task that runs as SYSTEM requires the calling process to already be elevated, which this attacker does not have, so the task is scoped to the compromised user's own session instead. The target location is outside the allow-listed installation directories, the parent process is not an approved orchestration tool, and `svcupd.exe` is unsigned and was created moments earlier by the same parent process. Every condition in Step 3's logic fires, and the correlation described below raises confidence further.",
    "SIMULATED true negative — host ALD-WKS-1187: an approved, baseline-allow-listed patch-management agent (identified by binary path plus cryptographic hash, not by process name) runs `schtasks.exe` to create task \"AgentMaintenance\", targeting the agent's own binary inside its allow-listed program-installation directory, scheduled daily. The target location is inside an allow-listed installation directory and the binary's hash matches the maintained baseline, so the suppression condition applies and no alert fires — this is intended behavior, not a missed detection.",
...
const module_: DetectionModule = {
  ...
  testCases: [
    "SIMULATED true positive — host ALD-WKS-2214, user m.romero: cmd.exe (parent: a document-viewer process) runs schtasks.exe to create task \"WinUpdaterHelper\", targeting svcupd.exe, an unsigned, freshly-created executable sitting in the user's per-account temporary-files directory, set to launch at logon under the compromised user's own account (not SYSTEM — this attacker only has user-level code execution, and a SYSTEM-run task requires an elevated caller). Target location is outside the allow-list, the parent process is not an approved orchestration tool, and the binary is unsigned and freshly created — all detection-logic conditions fire.",
    "SIMULATED true negative — host ALD-WKS-1187: an allow-listed patch-management agent (matched by path plus hash) runs schtasks.exe to create task \"AgentMaintenance\", targeting its own binary inside its allow-listed installation directory, scheduled daily. Target location is inside an allow-listed directory and the hash matches the baseline; the suppression condition applies and no alert fires.",
